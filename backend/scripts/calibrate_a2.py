"""Calibrate the A2 PedagogicalAgent on the same set of LM-18 syllabi as A1.

Mirrors scripts/calibrate_a1.py end to end — same five seuids, same
ChromaDB collection, same Vertex AI configuration, same audit format.
The only differences: it instantiates ``PedagogicalAgent`` (C3, C4)
and writes per-syllabus fixtures named ``a2_calibration_<seuid>.json``.

For each ``seuid`` the script:

1. Loads the Syllabus from the local SQLite DB.
2. Runs ``PedagogicalAgent.evaluate(syllabus)`` end-to-end against
...
When the A3/A4 agents land we will fold the three calibration scripts
into a single parametric ``calibrate_agent.py``; for now the
duplication is intentional and keeps each Phase 5.x committed
separately.
"""
...
from app.evaluation.agents.a2_learning_outcomes import PedagogicalAgent  # noqa: E402
...
def main(
    ...
    for seuid in seuids_to_run:
        ...
        agent = PedagogicalAgent(retriever=retriever, llm_client=capturing)
        ...
        out_path = output_dir / f"a2_calibration_{seuid}.json"
        out_path.write_text(
...
def _print_summary(rows: list[tuple[str, str, str, str, str]]) -> None:
    table = Table(title="A2 calibration summary", show_header=True)
