"""Calibrate the A1 CompletenessAgent on a fixed set of LM-18 syllabi.

For each ``seuid`` the script:

1. Loads the Syllabus from the local SQLite DB.
2. Runs ``CompletenessAgent.evaluate(syllabus)`` end-to-end against
   real Vertex AI (gemini-2.5-flash) and a real ChromaDB collection
   (the ingested normative corpus).
3. Saves a fixture under ``tests/fixtures/llm_responses/`` containing:
   - the syllabus snapshot used as input,
   - the final agent input (filtered fields + RAG context),
   - the full prompt sent to the LLM,
   - the raw LLM response text + its metadata (model, finish reason,
     latency, retry count, project_id),
   - the parsed AgentOutput (judgments + execution metadata),
   - or, on failure, a structured error block.

The fixtures are the calibration evidence: they let us inspect the
prompt/response pair offline and iterate the prompt without re-paying
for API calls.
"""
...
from app.evaluation.agents.a1_completeness import CompletenessAgent  # noqa: E402
...
def main(
    ...
    for seuid in seuids_to_run:
        ...
        agent = CompletenessAgent(retriever=retriever, llm_client=capturing)
        ...
        out_path = output_dir / f"a1_calibration_{seuid}.json"
        out_path.write_text(
...
def _print_summary(rows: list[tuple[str, str, str, str, str]]) -> None:
    table = Table(title="A1 calibration summary", show_header=True)
