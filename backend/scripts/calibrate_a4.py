"""Calibrate the A4 EditorialCareAgent on the same set of LM-18 syllabi.

Mirrors scripts/calibrate_a1.py / a2 / a3 end to end — same five
seuids, same ChromaDB collection, same Vertex AI configuration, same
audit format. The only differences: it instantiates
``EditorialCareAgent`` (C9 only) and writes per-syllabus fixtures
named ``a4_calibration_<seuid>.json``.

For each ``seuid`` the script:

1. Loads the Syllabus from the local SQLite DB.
2. Runs ``EditorialCareAgent.evaluate(syllabus)`` end-to-end against
...
A4 is the last of the four agents; once this script runs cleanly the
four calibration scripts (a1, a2, a3, a4) will be folded into a single
parametric ``calibrate_agent.py`` together with the LangGraph
orchestrator.
"""
...
from app.evaluation.agents.a4_editorial import EditorialCareAgent  # noqa: E402
...
def main(
    ...
    for seuid in seuids_to_run:
        ...
        agent = EditorialCareAgent(retriever=retriever, llm_client=capturing)
        ...
        out_path = output_dir / f"a4_calibration_{seuid}.json"
        out_path.write_text(
...
def _print_summary(rows: list[tuple[str, str, str, str, str]]) -> None:
    table = Table(title="A4 calibration summary", show_header=True)
